...
def run(model, classifier, dataloader, args):
    ...
    with torch.no_grad():
        ...
        for images, target, entire_images in tqdm(dataloader, unit_scale=args.batch_size):
            if args.with_mask:
                images, masks = images
                masks = masks.to(device=args.device, dtype=input_dtype)
            else:
                images = images
                masks=None
            images = images.to(device=args.device, dtype=input_dtype)
            target = target.to(args.device)
            entire_images = entire_images.to(device=args.device, dtype=input_dtype)

            with autocast():
                # predict
                output = model(original_image=entire_images, image=images, mask=masks, text=None)
