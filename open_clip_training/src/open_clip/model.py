...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, mask=None, normalize: bool = False, ori_image_features=None):
        if ori_image_features:
            return self.visual(image, m=mask, original_features=ori_image_features, bg_add_cls=True, fine_tune=True)
    ...
    def forward(
        ...
        image_features = self.encode_image(image, normalize=True) if image is not None else None  # [b, 768]
        text_features = self.encode_text(text, normalize=True) if text is not None else None  # [b, 768]
